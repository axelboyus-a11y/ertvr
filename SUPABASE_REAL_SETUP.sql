...
begin
  ...
  values (
    new.id,
    new.email,
    coalesce(new.raw_user_meta_data->>'full_name', new.raw_user_meta_data->>'username', split_part(new.email, '@', 1), 'RTN User'),
    coalesce(new.raw_user_meta_data->>'username', lower(regexp_replace(split_part(new.email, '@', 1), '[^a-zA-Z0-9_]', '', 'g')) || floor(random()*9999)::text)
  )
  on conflict (id) do nothing;
  return new;
end;
...
-- Password-only admin RPC for HTML admin panel.
-- For testing the admin password is 2026. Change this in all functions before real public launch.
create or replace function public.admin_check(p_admin_password text)
returns boolean
language sql
security definer
set search_path = public
stable
as $$
  select p_admin_password = '2026';
$$;

grant execute on function public.admin_check(text) to anon, authenticated;

create or replace function public.admin_snapshot(p_admin_password text)
returns jsonb
language plpgsql
security definer
set search_path = public
as $$
begin
  if not public.admin_check(p_admin_password) then
    raise exception 'Wrong admin password';
  end if;
  return jsonb_build_object(
    'profiles', coalesce((select jsonb_agg(to_jsonb(p) order by p.created_at desc) from public.profiles p), '[]'::jsonb),
    'posts', coalesce((select jsonb_agg(to_jsonb(po) || jsonb_build_object('profiles', to_jsonb(pr)) order by po.created_at desc) from public.posts po left join public.profiles pr on pr.id = po.user_id), '[]'::jsonb),
    'stories', coalesce((select jsonb_agg(to_jsonb(st) || jsonb_build_object('profiles', to_jsonb(pr)) order by st.created_at desc) from public.stories st left join public.profiles pr on pr.id = st.user_id), '[]'::jsonb),
    'badges', coalesce((select jsonb_agg(to_jsonb(br) || jsonb_build_object('profiles', to_jsonb(pr)) order by br.created_at desc) from public.badge_requests br left join public.profiles pr on pr.id = br.user_id), '[]'::jsonb),
    'reports', coalesce((select jsonb_agg(to_jsonb(rp) || jsonb_build_object('reporter', to_jsonb(pr)) order by rp.created_at desc) from public.reports rp left join public.profiles pr on pr.id = rp.reporter_id), '[]'::jsonb),
    'settings', coalesce((select to_jsonb(s) from public.app_settings s where id = 1), '{}'::jsonb)
  );
end;
$$;
...
create or replace function public.admin_set_verified(p_admin_password text, p_user_id uuid, p_value boolean)
returns text
language plpgsql
security definer
set search_path = public
as $$
begin
  if not public.admin_check(p_admin_password) then raise exception 'Wrong admin password'; end if;
  update public.profiles set is_verified = p_value, updated_at = now() where id = p_user_id;
  insert into public.notifications(user_id, actor_id, type, title, body)
  values (p_user_id, null, 'admin', case when p_value then 'Blue badge approved' else 'Blue badge removed' end, case when p_value then 'Admin approved your blue badge.' else 'Admin removed your blue badge.' end);
  return 'ok';
end;
$$;

grant execute on function public.admin_set_verified(text, uuid, boolean) to anon, authenticated;

create or replace function public.admin_set_banned(p_admin_password text, p_user_id uuid, p_value boolean)
returns text
language plpgsql
security definer
set search_path = public
...
begin
  if not public.admin_check(p_admin_password) then raise exception 'Wrong admin password'; end if;
  update public.profiles set is_banned = p_value, updated_at = now() where id = p_user_id;
  insert into public.notifications(user_id, actor_id, type, title, body)
  values (p_user_id, null, 'admin', case when p_value then 'Account banned' else 'Account unbanned' end, case when p_value then 'Admin banned your account.' else 'Your account is active again.' end);
  return 'ok';
end;
$$;

grant execute on function public.admin_set_banned(text, uuid, boolean) to anon, authenticated;

create or replace function public.admin_review_badge(p_admin_password text, p_request_id uuid, p_status text)
returns text
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select user_id into u from public.badge_requests where id = p_request_id;
  if u is null then return 'not_found'; end if;
  update public.badge_requests set status = p_status, updated_at = now() where id = p_request_id;
  if p_status = 'approved' then update public.profiles set is_verified = true where id = u; end if;
  insert into public.notifications(user_id, actor_id, type, title, body)
  values (u, null, 'admin', case when p_status='approved' then 'Blue badge approved' else 'Blue badge rejected' end, case when p_status='approved' then 'Your profile is now verified.' else 'Admin rejected your blue badge request.' end);
  return 'ok';
end;
$$;

grant execute on function public.admin_review_badge(text, uuid, text) to anon, authenticated;

create or replace function public.admin_delete_post(p_admin_password text, p_post_id uuid)
returns text
language plpgsql
security definer
set search_path = public
...
create or replace function public.admin_delete_story(p_admin_password text, p_story_id uuid)
returns text
language plpgsql
security definer
set search_path = public
...
grant execute on function public.admin_delete_story(text, uuid) to anon, authenticated;

create or replace function public.admin_resolve_report(p_admin_password text, p_report_id uuid, p_status text)
returns text
language plpgsql
security definer
set search_path = public
...
create or replace function public.admin_save_settings(p_admin_password text, p_app_name text, p_primary_color text)
returns text
language plpgsql
security definer
set search_path = public
...
begin
  if not public.admin_check(p_admin_password) then raise exception 'Wrong admin password'; end if;
  update public.app_settings set app_name = p_app_name, primary_color = p_primary_color, updated_at = now() where id = 1;
  return 'ok';
end;
$$;
...
create or replace function public.admin_send_notice(p_admin_password text, p_body text)
returns text
language plpgsql
security definer
set search_path = public
...
begin
  if not public.admin_check(p_admin_password) then raise exception 'Wrong admin password'; end if;
  insert into public.notifications(user_id, actor_id, type, title, body)
  select id, null, 'admin_notice', 'Admin notice', p_body from public.profiles where coalesce(is_banned,false) = false;
  return 'ok';
end;
$$;

grant execute on function public.admin_send_notice(text, text) to anon, authenticated;
